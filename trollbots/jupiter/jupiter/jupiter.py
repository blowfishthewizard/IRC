...
class identd(threading.Thread):
	...
	def _start(self):
		sock = socket.socket(socket.AF_INET6, socket.SOCK_STREAM) if self.ipv6 else socket.socket(socket.AF_INET, socket.SOCK_STREAM)
		sock.bind(('::', 113)) if self.ipv6 else sock.bind(('0.0.0.0', 113))
		sock.listen(5)
